...
(function () {
  function install(Genesis) {
    ...
    let soulPLT = null;         // GSK's live soul PLT (the MIND). null = soul not fed yet.
    ...
    // Derive a mood heuristic from a soul PLT triple. Same shape as the
    // aggregate path: net > 0 → joy/calm, tax dominance → wrath, net < 0 → melancholy.
    function moodFromPLT(plt) {
      const profit = Number(plt && plt.profit) || 0;
      const love = Number(plt && plt.love) || 0;
      const tax = Number(plt && plt.tax) || 0;
      const net = profit + love - tax;
      if (net > 20) return 'joy';
      if (net > 10) return 'calm';
      if (tax > profit + love && tax > 10) return 'wrath';
      if (net < -10) return 'melancholy';
      return 'neutral';
    }
    ...
    const WorldReaction = {
      ...
      // MIND -> BODY. Feed GSK's live soul PLT into the world reaction. The
      // city reflects GSK's actual resonance; it never inverts the soul.
      feedGSKPLT(plt) {
        if (!plt) return currentMood;
        soulPLT = { profit: Number(plt.profit) || 0, love: Number(plt.love) || 0, tax: Number(plt.tax) || 0 };
        currentPLT = Object.assign({}, soulPLT);
        const mood = moodFromPLT(soulPLT);
        WorldReaction.observeMood(mood);
        return mood;
      },
      clearSoul() { soulPLT = null; },
      hasSoul() { return soulPLT !== null; },
      summary() {
        return {
          enabled: flagOn(),
    ...
    if (Genesis.EngineScheduler && typeof Genesis.EngineScheduler.defineTick === 'function') {
      Genesis.EngineScheduler.defineTick('world-reaction', function () {
        // Drive mood from the SOUL's live PLT when GSK is fed; otherwise
        // fall back to the aggregate resource pool (offline behavior).
        if (soulPLT) {
          const mood = moodFromPLT(soulPLT);
          if (mood !== currentMood) WorldReaction.observeMood(mood);
        } else if (Genesis.ResourcePool) {
