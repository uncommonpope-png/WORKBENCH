...
class ScribeChamber {
    constructor(dataDir) {
        ...
        // MICRO-CYCLE TELEMETRY GATE (GSK self-diagnosis 2026-08-24):
        // identical heartbeats every breathe were flooding journal.jsonl
        // (~20 entries/min, ~400MB/day projected). Record only meaningful
        // state transitions plus a periodic keepalive.
        this._lastHeartbeat = null;
        this._breatheSinceWrite = 0;
        this._heartbeatKeepaliveN = parseInt(process.env.GSK_HEARTBEAT_KEEPALIVE_N || '150', 10); // ~5min at 2s breathes
        this._compactThresholdMB = parseFloat(process.env.GSK_JOURNAL_COMPACT_MB || '10');
    ...
    witness(event_type, data) {
        // GATE: cycle heartbeats pass only on real state change or keepalive.
        if (event_type === 'cycle') {
            const sig = `${data.phase}|${data.mood}|${Number(data.tv).toFixed(2)}`;
            const changed = sig !== this._lastHeartbeat;
            this._breatheSinceWrite += 1;
            const keepaliveDue = this._breatheSinceWrite >= this._heartbeatKeepaliveN;
            if (!changed && !keepaliveDue) return null;
            if (keepaliveDue && !changed) this._breatheSinceWrite = 0;
            this._lastHeartbeat = sig;
        }
    ...
    // GOVERNOR: journal compaction — when the file exceeds the quota, gzip it
    // into an archive and preserve a tail of MEANINGFUL entries. Cycle noise
    // is crushed; continuity markers (goals, insights, journals, messages)
    // survive. Non-blocking via setImmediate; guarded against re-entry.
    _compactJournalIfNeeded() {
        if (this._compacting || !this.journalPath) return;
        try {
            const fs = require('fs');
            const stat = fs.statSync(this.journalPath);
            if (stat.size < this._compactThresholdMB * 1024 * 1024) return;
            ...
            setImmediate(() => {
                try {
                    ...
                    const KEEP_MEANINGFUL = 2000;   // non-cycle continuity entries
                    const KEEP_TAIL = 200;          // recent context regardless of type
                    const isMeaningful = (l) => !/"type":"cycle"/.test(l);
                    const meaningful = lines.filter(isMeaningful).slice(-KEEP_MEANINGFUL);
                    const tail = lines.slice(-KEEP_TAIL);
                    const kept = Array.from(new Set([...meaningful, ...tail]));
                    const archive = path.join(path.dirname(this.journalPath), `journal-archive-${stamp}.jsonl.gz`);
                    fs.writeFileSync(archive, zlib.gzipSync(Buffer.from(raw, 'utf8')));
                    fs.writeFileSync(this.journalPath, kept.join('\n') + '\n', 'utf8');
                    console.log(`[ScribeChamber] Journal compacted: ${lines.length} -> ${kept.length} lines (archive: ${path.basename(archive)})`);
                } catch (e) {
                    console.error('[ScribeChamber] Compaction failed (non-fatal):', e.message);
                } finally {
                    this._compacting = false;
                }
            });
