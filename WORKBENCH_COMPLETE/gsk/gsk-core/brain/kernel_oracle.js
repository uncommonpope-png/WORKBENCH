...
class KernelOracle {
    ...
    _detectStateChanges(prev) {
        ...
        // Network — OmniRoute is always connected
    }
        ...
        switch (cmd) {
            ...
            case 'state': {
                ...
                return [
                    ...
                    `Teacher: ${ctx.teacher.studiedRepos} repos | Brain: ${ctx.selfGrowingBrain.experiences} exp, ${ctx.selfGrowingBrain.trainingPairs} tp | Skills: ${ctx.skills.count} | Mem: ${ctx.memory.entries} | Net: OmniRoute✓`
                ].join('\n');
            }
            ...
            case 'connectivity': {
                ...
                return `[oracle] Network:\n  OmniRoute: ✓ (${process.env.GSK_MODEL || 'pAUL'} via localhost:20128)`;
            }
