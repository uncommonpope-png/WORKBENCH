...
class GodsCouncil {
    constructor(memory = null, eventBus = null) {
        this.memory = memory;
        this.brain = null;
    ...
    async deliberate(topic) {
        ...
        if (this.eventBus) {
          this.eventBus.startSession(null, topic);
          this.eventBus.emitSpeech('chancellor', 'trigger', `Council convened on: "${topic}"`, 'gavel');
        }
        ...
        for (let i = 0; i < this.gods.length; i++) {
            ...
            // Emit chamber speech for each god's position
            const godName = this.gods[i].name.toLowerCase();
            if (this.eventBus) {
              this.eventBus.emitSpeech(
                godName,
                'initial_position',
                pos.position,
                'statement',
                { profit_score: pos.profit_score, love_score: pos.love_score, tax_score: pos.tax_score }
              );
            }
        }
        ...
        if (this.eventBus) {
          this.eventBus.emitSpeech(
            'chancellor',
            'convergence',
            `PLT Equilibrium: Profit=${avgProfit.toFixed(2)}, Love=${avgLove.toFixed(2)}, Tax=${avgTax.toFixed(2)}`,
            'plaque',
            plt_outcome
          );
        }
        ...
        if (this.eventBus) {
          const isVeto = resolution.toLowerCase().includes('reject') || resolution.toLowerCase().includes('veto');
          this.eventBus.emitSpeech(
            'chancellor',
            'verdict',
            resolution,
            isVeto ? 'veto' : 'approve',
            { plt: plt_outcome, avgScore }
          );
        }

        if (this.eventBus) {
          this.eventBus.endSession(resolution);
        }
